...
const productSchema = new mongoose.Schema({
  ...
  slug: { type: String, trim: true, lowercase: true, unique: true, sparse: true, maxlength: 140 },
...
productSchema.pre('validate', async function(next) {
  ...
  const existing = await this.constructor.findOne({
    slug: { $regex: `^${base}(?:-[a-f0-9]{6})?$`, $options: 'i' },
    _id: { $ne: this._id }
  }).select('_id').lean();
  this.slug = existing ? `${base}-${String(this._id).slice(-6).toLowerCase()}` : base;
  next();
});

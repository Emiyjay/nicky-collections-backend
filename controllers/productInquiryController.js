...
const extensionForType = (contentType = '') => ({ 'image/jpeg': 'jpg', 'image/png': 'png', 'image/webp': 'webp', 'image/gif': 'gif' })[contentType.split(';')[0].trim().toLowerCase()] || 'jpg';
...
const fetchImageAttachments = async (images) => {
  ...
  for (let index = 0; index < Math.min(images.length, MAX_IMAGE_ATTACHMENTS); index += 1) {
    ...
    } catch (error) { console.warn(`Product image attachment ${index + 1} could not be fetched:`, error.message); }
  }
  return attachments;
};
...
exports.createProductInquiry = async (req, res) => {
  try {
    ...
    const product = /^[a-f0-9]{24}$/i.test(identifier) ? await Product.findById(identifier).select('-reviews').lean() : await Product.findOne({ slug: identifier }).select('-reviews').lean();
    ...
    if (!name) return res.status(400).json({ message: 'Please provide your full name.' });
    if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return res.status(400).json({ message: 'Please provide a valid email address.' });
    if (!location) return res.status(400).json({ message: 'Please provide your delivery location.' });
    if (!product.inStock) return res.status(409).json({ message: 'This product is currently out of stock.' });
    if (Number(product.stockCount) > 0 && requestedQuantity > Number(product.stockCount)) return res.status(409).json({ message: `Only ${product.stockCount} unit${Number(product.stockCount) === 1 ? '' : 's'} are currently available.` });
